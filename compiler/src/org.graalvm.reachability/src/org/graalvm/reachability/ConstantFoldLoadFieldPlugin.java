...
/**
 * Plugin for the bytecode parser that performs constant folding during parsing. This is a bit
 * earlier than constant folding done by the {@link CanonicalizerPhase} that optimizes the graph
 * after parsing. It allows other plugins that run during parsing, like method intrinsifications, to
 * see more constant values.
 */
final class ConstantFoldLoadFieldPlugin implements NodePlugin {

    @Override
    public boolean handleLoadField(GraphBuilderContext b, ValueNode receiver, ResolvedJavaField field) {
        return receiver.isConstant() && tryConstantFold(b, field, receiver.asJavaConstant());
    }
